...
interface ConnectingAudioVisualizerProps {
  subscribeToVolume: (callback: (volume: number) => void) => () => void;
}

export const ConnectingAudioVisualizer: React.FC<ConnectingAudioVisualizerProps> = ({ subscribeToVolume }) => {
  const [isListening, setIsListening] = useState(false);

  useEffect(() => {
    const unsubscribe = subscribeToVolume((vol) => {
      // Use a small threshold to avoid flickering from background noise
      const currentlyListening = vol > 0.01;
